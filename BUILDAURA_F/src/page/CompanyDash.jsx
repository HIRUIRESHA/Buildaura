...
import { getCompanies, getCompanyProfile } from "../services/companyServices";
import { getCompanyProjects, updateProjectStatus } from "../services/projectCartServices";
import { getCompanyEngineers } from "../services/userServices"; // ✅ Import engineers API

const CompanyDash = () => {
  const [profile, setProfile] = useState(null);
  const [projects, setProjects] = useState([]);
  const [engineers, setEngineers] = useState([]); // ✅ New state for engineers
  const [modalOpen, setModalOpen] = useState(false);
  const [modalContent, setModalContent] = useState(null);
  ...
  useEffect(() => {
    fetchProfile();
    fetchProjects();
    fetchEngineers(); // ✅ Fetch engineers on mount
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-8">
        ...
        {/* Engineers Section - Beautiful Design */}
        {engineers.length > 0 && (
